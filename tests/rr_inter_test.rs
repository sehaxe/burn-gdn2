...
#[test]
fn rr_inter_matches_real_inter() {
    const HS: usize = 512;
    const HEADS: usize = 2;
    const HK: usize = 128;
    const CS: usize = 64;
    ...
    let ref_out = inter_launch_raw::<Bare>(
        io.aqk.clone(),
        io.w.clone(),
        io.u.clone(),
        io.qgt.clone(),
    ...
    let _state_out_h = client.empty(HEADS * KD * VD * 4);
    unsafe {
        rr_inter_kernel::launch(
            &client,
            CubeCount::Static(HEADS as u32, (VD / 16) as u32, 1),
            CubeDim { x: 32, y: 1, z: 1 },
            BufferArg::from_raw_parts(w_c.handle, HEADS * NT * KD * C),
            BufferArg::from_raw_parts(u_c.handle, HEADS * NT * C * VD),
            BufferArg::from_raw_parts(qg_c.handle, HEADS * NT * KD * C),
            BufferArg::from_raw_parts(kgd_c.handle, HEADS * NT * C * KD),
            BufferArg::from_raw_parts(aqk_c.handle, HEADS * NT * C * C),
            BufferArg::from_raw_parts(glast_c.handle, HEADS * NT * KD),
            BufferArg::from_raw_parts(st.handle.clone(), HEADS * KD * VD),
            BufferArg::from_raw_parts(_state_out_h.clone(), HEADS * KD * VD),
    ...
    let got: Vec<f32> = raw
        ...
        .collect();
    let ref_d = ref_out.into_data();
    let mut d = 0.0f32;
    for (i, g) in got.iter().enumerate() {
        let e = f32::from_le_bytes(ref_d.bytes[i * 4..i * 4 + 4].try_into().unwrap());
        d = d.max((g - e).abs());
    }
    println!("rr_inter vs real inter: out_diff={d:.3e}");
    assert!(d < 5e-2, "rr_inter out broken: {d:.3e}");
}

#[test]
fn rr_inter_state_matches_cpu() {
    const HS: usize = 512;
    const HEADS: usize = 1;
    const HK: usize = 128;
    const CS: usize = 64;
    ...
    const KD: usize = HK;
    use burn::tensor::{Device, Distribution, Tensor};
    use burn_gdn2::kernel::chunk_cube::cuda::intra_launch_raw;
    use burn_gdn2::Gdn2Config;
    type Bare = burn_gdn2::CudaBare;
    let dev = Device::cuda(0);
    let cfg = Gdn2Config {
    ...
    let got_s: Vec<f32> = raw_s
        ...
        .map(|x| f32::from_le_bytes(x.try_into().unwrap()))
        .collect();
    // CPU: S_new[k][v] = sum_r kgd[r][k] * u[r][v]  (state=0, NT=1, vn=u)
    let kgd_d = io.kgd.clone().into_data();
    let u_d = io.u.clone().into_data();
    let rd = |d: &burn::tensor::TensorData, i: usize| {
        f32::from_le_bytes(d.bytes[i * 4..i * 4 + 4].try_into().unwrap())
    };
    let mut d = 0.0f32;
    for k in 0..KD {
        for v in 0..VD {
            let mut acc = 0.0f32;
            for r in 0..C {
                acc += rd(&kgd_d, r * KD + k) * rd(&u_d, r * VD + v);
            }
            d = d.max((got_s[k * VD + v] - acc).abs());
        }
    }
    println!("rr state vs cpu kgd@u: max_diff={d:.3e}");
    assert!(d < 2e-2, "rr state broken: {d:.3e}");
}
